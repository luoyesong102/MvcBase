...
namespace Tpo.RedisSessionStateStore
{
    /// <summary>
    ...
    ///              type="Tpo.RedisSessionStateStore.RedisSessionStateStoreProvider" 
    ...
    public sealed class RedisSessionStateStoreProvider : SessionStateStoreProviderBase
    {
        ...
        private string name; 
        public static void InitialRedisSession()
        {
            clientManager = new PooledRedisClientManager(ConfigurationManager.AppSettings["redisCacheAddress"]);
            RedisSessionStateStoreProvider.SetClientManager(clientManager);
        ...
        //HttpCookie co = Request.Cookies["ASP.NET_SessionId"];  co.Domain = ".mysite.com"; Request.Cookies.Set( co )
        private string GetSessionIdKey(string id)
        {
            return name + options.KeySeparator + id;
        }

        private string GetSessionIdKey(HttpContext context, string id)
        {
            //HttpCookie co = context.Request.Cookies["ASP.NET_SessionId"];
            //if (co != null)
            //{
            //    co.Domain = ".localcrm.com";
            //}
            //context.Request.Cookies.Set(co);
        ...
        int tempSessionTimeout = 0;
        int GetTimeoutMinutes(HttpSessionState session)
        {
            if (tempSessionTimeout != 0) return tempSessionTimeout;
            //默认超时20分钟
            int outMinutes=20;
            if (session == null || !int.TryParse(session.Timeout+"", out outMinutes))
                outMinutes = 20;
            return outMinutes;
        }
        ...
        public override void ResetItemTimeout(HttpContext context, string id)
        {
            ...
            using (var client = GetClient())
            {
                ...
                {
                    transaction.QueueCommand(c => c.ExpireEntryIn(key, TimeSpan.FromMinutes(GetTimeoutMinutes(context.Session))));
                });
            };
        }
        ...
        public override void ReleaseItemExclusive(HttpContext context, string id, object lockId)
        {
            using (var client = GetClient())
            {
                ...
                {
                    state.Locked = false;
                    state.Timeout = GetTimeoutMinutes(context.Session);//context.Session.Timeout;
                });
            }
        }
