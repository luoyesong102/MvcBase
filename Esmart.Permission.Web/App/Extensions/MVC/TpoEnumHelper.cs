...
{
    public static class TpoEnumHelper
    {
        /// <summary>
        ...
        /// <see cref="Type"/> and <see cref="SelectExtensions.EnumDropDownListFor{TModel,TEnum}(HtmlHelper, Expression{Func{TModel, TEnum}})"/>
        ...
        public static IList<SelectListItem> GetSelectList(Type type)
        {
            ...
            if (!IsValidForEnumHelper(type))
            {
                throw new ArgumentException(
                    string.Format(CultureInfo.CurrentCulture, "Type '{0}' is not supported.", type.FullName), "type");
            }
        ...
        public static IList<SelectListItem> GetSelectList(ModelMetadata metadata)
        {
            ...
            {
                throw new ArgumentException( "ModelType property must not be null." ,"metadata");
            }

            if (!IsValidForEnumHelper(metadata))
            {
                throw new ArgumentException(
                    String.Format( CultureInfo.CurrentCulture, "Type '{0}' is not supported.",
                        metadata.ModelType.FullName ), "metadata" );
            }
            ...
            if (valueType != null && valueType != type && valueType != Nullable.GetUnderlyingType(type))
            {
                throw new ArgumentException(
                    String.Format(CultureInfo.CurrentCulture,
                        "Invalid value parameter type '{0}'. Must match type parameter '{1}'.",
                        valueType.FullName, type.FullName), "value");
            }
        ...
        public static string GetDisplayName(FieldInfo field)
        {
            ...
            var displays = field.GetCustomAttributes(typeof (DisplayAttribute), false);
            if (displays.Any())
            {
